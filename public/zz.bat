...
for /f "delims=" %%i in ('dir /S /B /A-D ^| findstr /v /i "\\node_modules\\" ^| findstr /v /i "\\.next\\" ^| findstr /v /i "\\.git\\" ^| findstr /v /i "\\.ssh\\" ^| findstr /v /i ".jpeg" ^| findstr /v /i ".gif" ^| findstr /v /i ".bmp" ^| findstr /v /i ".svg" ^| findstr /v /i ".bat" ^| findstr /v /i ".py" ^| findstr /r /v "^[^.]*$"') do echo %%i >> project-structure.txt
